...
export default function SingleNote({note}) {
  ...
  return (
    ...
    <div className="p-4 transition-all rounded-[11px] dark:bg-first dark:hover:bg-second">
        ...
        <div className="block md:hidden md:mb-2">
            <div className="mb-2">
                <span className="px-2 py-1 text-sm rounded-full bg-eighth text-fifth">{note.attributes?.categories?.data[0].attributes.title}</span>
                <span className="ml-2 text-sm text-fourth">{note.attributes.views} views</span>
            </div>
            <span className="block text-lg font-bold">{note.attributes.title}</span>
        </div>
        <p className="prose dark:prose-invert">{note.attributes.body.replace(regex,'').substring(0, 120)}...</p>
    </div>
  )
}
