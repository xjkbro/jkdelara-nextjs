...
export default function SingleNote({ note, i }) {
    ...
    return (
        <motion.div
            ...
            // className="animate-border rounded-xl bg-white from-teal-500 via-purple-500 h-fit md:h-[10rem] to-pink-500 bg-[length:400%_400%] p-1 transition bg-gradient-to-r"
        >
            ...
            <div className="p-4 transition-all rounded-[11px] h-fit md:h-[9.5rem] dark:bg-first dark:hover:bg-second bg-fifth hover:bg-white">
                <div className="hidden md:block md:mb-2">
                    <span className="px-2 py-1 text-sm font-bold rounded-full bg-eighth text-fifth">
                        {note.attributes?.categories?.data[0].attributes.title}
                    </span>
                    <span className="ml-2 text-lg font-bold">
                        {note.attributes.title}
                    </span>
                    <span className="ml-2 text-sm dark:text-fourth whitespace-nowrap">
