...
export default function SingleNote({ note, i }) {
    ...
    return (
        <motion.div
            ...
            className="animate-border rounded-xl bg-white from-teal-500 via-purple-500 h-[10rem] to-pink-500 bg-[length:400%_400%] p-1 transition bg-gradient-to-r" >
            ...
            <div className="p-4 transition-all rounded-[11px] h-[9.5rem] dark:bg-first dark:hover:bg-second bg-fifth hover:bg-white">
                ...
                <p className="prose dark:prose-invert">{(note.attributes.summary.length > 200) ? <>{note.attributes.summary.substring(0, 200)}...</> : <>{note.attributes.summary}</>}</p>
            </div>
        </motion.div>
    )
}
